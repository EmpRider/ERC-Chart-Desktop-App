...
| --- | --- | --- | --- |
| `format:check` | ECDD-56 | Windows | Exit `0` only when formatting is compliant; do not modify files. |
| `lint` | ECDD-56 | Windows | Exit `0` only when strict lint rules pass. |
| `typecheck` | ECDD-56 | Windows | Exit `0` only when the complete TypeScript workspace type-checks. |
| `test:unit` | ECDD-56 | Windows | Run deterministic unit tests and fail on any test failure. |
...
| `audit:ci` | ECDD-56 | Windows | Run the approved dependency/security audit and fail blocking findings. |
...
- PR application validation is Windows-only.
