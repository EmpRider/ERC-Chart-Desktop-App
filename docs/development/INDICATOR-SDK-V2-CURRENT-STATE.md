...
| Authoring docs              | `INDICATOR-AUTHORING.md` documents legacy API support, positional/unconditional restrictions, manual keys, manual drawing IDs/scopes, and bounded custom history helpers as normal usage.                                                                                       | Rewrite around the v2-only contract after APIs stabilize. Document the ECDD-142 source/MTF ownership boundary without removing those concepts from the overall SDK v2 architecture.                                                    | ECDD-226, ECDD-142 boundary                            |
...
9. **Documentation — ECDD-226.** Publish the final v2 authoring model and document that provider-aware MTF acquisition/per-TA overrides are delivered by ECDD-142 while remaining part of the overall SDK v2 architecture.
