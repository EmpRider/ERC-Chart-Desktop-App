...
test("defines Development Version 2 release identity", () => {
  assert.equal(applicationVersion, "0.1.0-dev.2");
  assert.equal(packageIdentityName, "erc-chart-desktop-app");
  assert.equal(releaseTag(applicationVersion), "v0.1.0-dev.2");
  assert.equal(
    installerArtifactName(applicationVersion),
    "ERC-Chart-Setup-0.1.0-dev.2.exe",
  );
});
...
test("requires the packaged ASAR manifest to carry the release version", () => {
  assert.doesNotThrow(() => assertPackagedVersion("0.1.0-dev.2"));
  assert.throws(
...
test("writes a conventional SHA-256 checksum line", () => {
  assert.equal(
    checksumLine("a".repeat(64), "ERC-Chart-Setup-0.1.0-dev.2.exe"),
    `${"a".repeat(64)}  ERC-Chart-Setup-0.1.0-dev.2.exe\n`,
  );
