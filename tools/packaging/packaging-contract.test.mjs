...
test("defines klinecharts architecture release identity", () => {
  assert.equal(applicationVersion, "0.3.4");
  assert.equal(packageIdentityName, "erc-chart-desktop-app");
  assert.equal(releaseTag(applicationVersion), "v0.3.4");
  assert.equal(
    installerArtifactName(applicationVersion),
    "ERC-Chart-Setup-0.3.4.exe",
  );
});
...
test("requires the packaged ASAR manifest to carry the release version", () => {
  assert.doesNotThrow(() => assertPackagedVersion("0.3.4"));
  assert.throws(
...
test("writes a conventional SHA-256 checksum line", () => {
  assert.equal(
    checksumLine("a".repeat(64), "ERC-Chart-Setup-0.3.4.exe"),
    `${"a".repeat(64)}  ERC-Chart-Setup-0.3.4.exe\n`,
  );
