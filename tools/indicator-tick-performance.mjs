...
const indicatorBuildRoot = await mkdtemp(
  path.join(os.tmpdir(), "erc-indicator-tick-performance-"),
);
const builtIndicator = await buildAtrRopeUtBotIndicatorPackage({
  root: path.resolve(import.meta.dirname, ".."),
  outputRoot: path.join(indicatorBuildRoot, "package"),
});
const { default: atrRopeUtBotIndicator } = await import(
  `${pathToFileURL(path.join(builtIndicator.packageRoot, "dist", "index.js")).href}?performance=${Date.now()}`
);
...
try {
  ...
  await rm(indicatorBuildRoot, { recursive: true, force: true });
}
