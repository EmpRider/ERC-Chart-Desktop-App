...
test("plugin wrapper filters outside files and preserves loader selection", async (t) => {
  const sourceRoot = await mkdtemp(path.join(os.tmpdir(), "erc-history-plugin-root-"));
  const outsideRoot = await mkdtemp(path.join(os.tmpdir(), "erc-history-plugin-outside-"));
...
test("package build transforms indicator modules across the authoring package root", async (t) => {
  const authoringRoot = await mkdtemp(path.join(os.tmpdir(), "erc-history-authoring-root-"));
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), "erc-history-authoring-output-"));
  ...
  const entry = await readFile(path.join(result.packageRoot, result.manifest.entry), "utf8");
